...
def load_config_files(agents: str, zones: str):
    ...
    out_path = (Path(__file__).resolve().parent / ".." / "isaac_utils" / "config" / "agent_data_gen.yaml").resolve()
...
def main():
    ...
    agent_path = (base_dir / ".." / "isaac_utils" / "config" / "agents.yaml").resolve()
    zone_path  = (base_dir / ".." / "isaac_utils" / "config" / "zones.yaml").resolve()

    load_config_files(str(agent_path), str(zone_path))
